...
function Proposal({ R }) {
  ...
  const exportPptx = async () => {
    ...
    try {
      ...
      pres.author = "Allium Sales Intelligence";
      pres.title = "Allium × " + co.name + " — Data Partnership Proposal";

      const BG = "0A0E1A"; const SF = "111827"; const TX = "E2E8F0"; const TM = "7C869B"; const AC = "3B82F6"; const GN = "10B981"; const AM = "F59E0B"; const RD = "EF4444"; const PU = "8B5CF6";
      const mkShadow = () => ({ type: "outer", blur: 4, offset: 2, angle: 135, color: "000000", opacity: 0.2 });

      // SLIDE 1: Title
      let s = pres.addSlide(); s.background = { color: BG };
      s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 0, w: 10, h: 5.625, fill: { color: "0F172A" } });
      s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 4.4, w: 10, h: 1.225, fill: { color: AC, transparency: 85 } });
      s.addText("DATA PARTNERSHIP PROPOSAL", { x: 0.8, y: 1.2, w: 8, h: 0.4, fontSize: 12, fontFace: "Arial", color: AC, charSpacing: 6, bold: true });
      s.addText("Allium × " + co.name, { x: 0.8, y: 1.7, w: 8.5, h: 1, fontSize: 38, fontFace: "Arial Black", color: "FFFFFF", bold: true });
      s.addText("Prepared " + new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) + "  —  Confidential", { x: 0.8, y: 2.8, w: 8, h: 0.4, fontSize: 13, fontFace: "Arial", color: TM });
      s.addText(ucs.slice(0, 3).map(u => u.cat).join("  ·  "), { x: 0.8, y: 3.4, w: 8, h: 0.4, fontSize: 12, fontFace: "Arial", color: GN, bold: true });
      s.addText("allium.so", { x: 0.8, y: 4.7, w: 3, h: 0.4, fontSize: 11, fontFace: "Arial", color: TM });

      // SLIDE 2: Executive Summary
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Executive Summary", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      s.addText(co.name + " operates in " + (co.industry || "the blockchain space") + ". Allium proposes a data partnership providing production-grade infrastructure across 80+ blockchains with sub-second freshness.", { x: 0.6, y: 1.1, w: 8.8, h: 0.9, fontSize: 13, fontFace: "Arial", color: TM, lineSpacingMultiple: 1.4 });
      const stats = [["80+", "Blockchains"], ["<5s", "Freshness"], ["90K+", "Peak RPS"], ["16+", "Enterprise Logos"]];
      stats.forEach(([v, l], i) => {
        const sx = 0.6 + i * 2.3;
        s.addShape(pres.shapes.RECTANGLE, { x: sx, y: 2.3, w: 2.1, h: 1.2, fill: { color: SF }, shadow: mkShadow() });
        s.addText(v, { x: sx, y: 2.4, w: 2.1, h: 0.7, fontSize: 28, fontFace: "Arial Black", color: AC, align: "center", bold: true });
        s.addText(l, { x: sx, y: 3.0, w: 2.1, h: 0.4, fontSize: 10, fontFace: "Arial", color: TM, align: "center" });
      });

      // SLIDE 3: Challenges
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Challenges We Solve", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      const pains = co.painPoints || ["Managing blockchain data at scale", "Data freshness and reliability", "Multi-chain complexity", "Engineering time on pipelines"];
      pains.forEach((p, i) => {
        const px = 0.6 + (i % 2) * 4.5; const py = 1.2 + Math.floor(i / 2) * 1.6;
        s.addShape(pres.shapes.RECTANGLE, { x: px, y: py, w: 4.2, h: 1.2, fill: { color: SF }, shadow: mkShadow() });
        s.addShape(pres.shapes.RECTANGLE, { x: px, y: py, w: 0.06, h: 1.2, fill: { color: AM } });
        s.addText(p, { x: px + 0.3, y: py + 0.15, w: 3.6, h: 0.9, fontSize: 13, fontFace: "Arial", color: TX, valign: "middle" });
      });

      // SLIDE 4: Product Stack
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Recommended Product Stack", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      prods.forEach((p, i) => {
        const py = 1.1 + i * 1.0;
        s.addShape(pres.shapes.RECTANGLE, { x: 0.6, y: py, w: 8.8, h: 0.85, fill: { color: SF }, shadow: mkShadow() });
        s.addShape(pres.shapes.RECTANGLE, { x: 0.6, y: py, w: 0.06, h: 0.85, fill: { color: AC } });
        s.addText(p.name, { x: 0.9, y: py + 0.05, w: 4, h: 0.4, fontSize: 14, fontFace: "Arial", color: TX, bold: true });
        s.addText(p.fresh || "", { x: 7.8, y: py + 0.05, w: 1.4, h: 0.35, fontSize: 9, fontFace: "Arial", color: GN, align: "center", bold: true });
        s.addText(p.desc || "", { x: 0.9, y: py + 0.4, w: 7.5, h: 0.35, fontSize: 10, fontFace: "Arial", color: TM });
      });

      // SLIDE 5: Product Comparison Table
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Product Comparison", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      const tblHeader = [{ text: "", options: { fill: { color: SF }, color: TM, bold: true } }, ...["Explorer", "Datashares", "Realtime APIs", "Datastreams"].map(h => ({ text: h, options: { fill: { color: SF }, color: TX, bold: true, align: "center" } }))];
      const tblRows = [["Freshness", "~1 hr", "1-3 hrs", "3-5s", "3-5s"], ["Chains", "80+", "80+", "20+", "80+"], ["Interface", "SQL/App", "Snowflake/BQ/S3", "REST API", "Kafka/PubSub"], ["Best For", "Analytics", "Data Lake", "Prod Apps", "Pipelines"]];
      const tblData = [tblHeader, ...tblRows.map(row => [{ text: row[0], options: { fill: { color: "0D1117" }, color: TM, bold: true } }, ...row.slice(1).map(c => ({ text: c, options: { fill: { color: "0D1117" }, color: TX, align: "center" } }))])];
      s.addTable(tblData, { x: 0.6, y: 1.2, w: 8.8, colW: [1.8, 1.75, 1.75, 1.75, 1.75], border: { pt: 0.5, color: "1E293B" }, fontSize: 11, fontFace: "Arial" });

      // SLIDE 6: Chain Coverage
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Blockchain Coverage", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      s.addText("115+ chains indexed  ·  3,000+ tables  ·  13 ecosystems", { x: 0.6, y: 0.85, w: 8, h: 0.3, fontSize: 11, fontFace: "Arial", color: TM });
      const topChains = ALLIUM_CHAINS.slice(0, 24);
      topChains.forEach((ch, i) => {
        const cx = 0.6 + (i % 6) * 1.55; const cy = 1.4 + Math.floor(i / 6) * 0.95;
        const ecCol = (ECO_COLORS[ch.eco] || "#3b82f6").replace("#", "");
        s.addShape(pres.shapes.RECTANGLE, { x: cx, y: cy, w: 1.4, h: 0.75, fill: { color: SF }, shadow: mkShadow() });
        s.addShape(pres.shapes.RECTANGLE, { x: cx, y: cy, w: 0.05, h: 0.75, fill: { color: ecCol } });
        s.addText(ch.name, { x: cx + 0.15, y: cy + 0.05, w: 1.0, h: 0.35, fontSize: 9, fontFace: "Arial", color: TX, bold: true });
        s.addText(ch.tables.toString(), { x: cx + 0.95, y: cy + 0.05, w: 0.35, h: 0.35, fontSize: 9, fontFace: "Arial", color: TM, align: "right" });
        s.addText(ch.eco + " · " + ch.layer, { x: cx + 0.15, y: cy + 0.4, w: 1.1, h: 0.25, fontSize: 7, fontFace: "Arial", color: TM });
      });

      // SLIDE 7: Social Proof
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Trusted by Industry Leaders", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      KB.customers.forEach((c, i) => {
        const cx = 0.6 + (i % 4) * 2.35; const cy = 1.1 + Math.floor(i / 4) * 0.85;
        const isMatch = cust.some(x => x.n === c.n);
        s.addShape(pres.shapes.RECTANGLE, { x: cx, y: cy, w: 2.15, h: 0.7, fill: { color: SF }, shadow: mkShadow() });
        if (isMatch) s.addShape(pres.shapes.RECTANGLE, { x: cx, y: cy, w: 0.05, h: 0.7, fill: { color: AC } });
        s.addText(c.n, { x: cx + 0.15, y: cy + 0.05, w: 1.9, h: 0.3, fontSize: 11, fontFace: "Arial", color: isMatch ? AC : TX, bold: true });
        s.addText(c.u, { x: cx + 0.15, y: cy + 0.35, w: 1.9, h: 0.25, fontSize: 7, fontFace: "Arial", color: TM });
      });
      // Case studies
      s.addText([
        { text: "Phantom: ", options: { bold: true, color: AC } }, { text: "15M MAUs, 90K RPS, 477M requests in 4hrs", options: { color: TM, breakLine: true } },
        { text: "Wormhole: ", options: { bold: true, color: AC } }, { text: "$100M+ saved via sybil detection for airdrop", options: { color: TM, breakLine: true } },
        { text: "Uniswap Foundation: ", options: { bold: true, color: AC } }, { text: "v4 adoption dashboard (dexanalytics.org)", options: { color: TM } },
      ], { x: 0.6, y: 4.5, w: 8.8, h: 0.9, fontSize: 9, fontFace: "Arial" });

      // SLIDE 8: Competitive
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Why Allium Over Alternatives", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      [{ vs: "vs. In-House", pts: ["80+ chains from day 1", "No infra maintenance", "Engineering time → product"] }, { vs: "vs. Dune/Flipside", pts: ["Production SLAs", "Sub-second freshness", "Your own Snowflake"] }, { vs: "vs. The Graph", pts: ["No subgraph dev", "80+ chains, one SQL", "Enriched schemas"] }, { vs: "vs. Chainlink/Moralis", pts: ["Full historical + realtime", "Cross-chain joins", "Enterprise Datashare"] }].forEach((item, i) => {
        const cx = 0.6 + (i % 2) * 4.65; const cy = 1.1 + Math.floor(i / 2) * 2.0;
        s.addShape(pres.shapes.RECTANGLE, { x: cx, y: cy, w: 4.35, h: 1.7, fill: { color: SF }, shadow: mkShadow() });
        s.addText(item.vs, { x: cx + 0.2, y: cy + 0.1, w: 3.8, h: 0.35, fontSize: 13, fontFace: "Arial", color: AC, bold: true });
        s.addText(item.pts.map(p => ({ text: p, options: { bullet: true, breakLine: true, color: TX } })), { x: cx + 0.2, y: cy + 0.5, w: 3.8, h: 1.0, fontSize: 11, fontFace: "Arial" });
      });

      // SLIDE 9: Timeline
      s = pres.addSlide(); s.background = { color: BG };
      s.addText("Implementation Timeline", { x: 0.6, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      [{ w: "Day 1", t: "Kickoff & Access", d: "Explorer access, API keys, Datashare connections provisioned." }, { w: "Week 1", t: "Integration & POC", d: "Connect to your stack. Proof-of-concept against live data." }, { w: "Week 2-3", t: "Production Deploy", d: "Schema customization, query optimization, SLA setup." }, { w: "Week 4+", t: "Scale & Optimize", d: "New chains, custom schemas, QBRs. 24/7 support." }].forEach((step, i) => {
        const sy = 1.2 + i * 1.0;
        s.addShape(pres.shapes.OVAL, { x: 0.85, y: sy + 0.1, w: 0.25, h: 0.25, fill: { color: AC } });
        if (i < 3) s.addShape(pres.shapes.LINE, { x: 0.97, y: sy + 0.35, w: 0, h: 0.75, line: { color: AC, width: 1.5 } });
        s.addText(step.w, { x: 1.3, y: sy, w: 1.2, h: 0.35, fontSize: 10, fontFace: "Arial", color: AC, bold: true });
        s.addText(step.t, { x: 2.5, y: sy, w: 4, h: 0.35, fontSize: 14, fontFace: "Arial", color: TX, bold: true });
        s.addText(step.d, { x: 2.5, y: sy + 0.35, w: 6, h: 0.35, fontSize: 10, fontFace: "Arial", color: TM });
      });

      // SLIDE 10: Next Steps / CTA
      s = pres.addSlide(); s.background = { color: BG };
      s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 0, w: 10, h: 5.625, fill: { color: "0F172A" } });
      s.addText("Next Steps", { x: 0.6, y: 0.8, w: 9, h: 0.7, fontSize: 32, fontFace: "Arial Black", color: TX, bold: true, margin: 0 });
      [{ n: "1", t: "15-Min Discovery", d: "Map " + co.name + "'s data needs to Allium capabilities" }, { n: "2", t: "Live Demo", d: "Query your specific chains and tokens in real-time" }, { n: "3", t: "POC / Trial", d: "Free trial to validate quality, freshness, and performance" }].forEach((step, i) => {
        const sx = 0.6 + i * 3.1;
        s.addShape(pres.shapes.RECTANGLE, { x: sx, y: 2.0, w: 2.8, h: 2.0, fill: { color: SF }, shadow: mkShadow() });
        s.addShape(pres.shapes.OVAL, { x: sx + 1.05, y: 2.2, w: 0.6, h: 0.6, fill: { color: AC } });
        s.addText(step.n, { x: sx + 1.05, y: 2.22, w: 0.6, h: 0.6, fontSize: 20, fontFace: "Arial Black", color: "FFFFFF", align: "center", valign: "middle", bold: true });
        s.addText(step.t, { x: sx + 0.15, y: 3.0, w: 2.5, h: 0.4, fontSize: 13, fontFace: "Arial", color: TX, align: "center", bold: true });
        s.addText(step.d, { x: sx + 0.15, y: 3.4, w: 2.5, h: 0.5, fontSize: 10, fontFace: "Arial", color: TM, align: "center" });
      });
      s.addText("allium.so/contact", { x: 3, y: 4.5, w: 4, h: 0.5, fontSize: 16, fontFace: "Arial", color: AC, align: "center", bold: true, hyperlink: { url: "https://allium.so/contact" } });

      await pres.writeFile({ fileName: "Allium_x_" + co.name.replace(/[^a-zA-Z0-9]/g, "_") + "_Proposal.pptx" });
    } catch (e) {
  ...
  return <div className="fu" style={{ maxWidth: 900, margin: "0 auto" }}>
    ...
    <div style={{ background: "linear-gradient(135deg,#0c1029,#0f1a3a,#0c1029)", border: "1px solid "+C.ac+"30", borderRadius: 16, padding: "32px 28px", marginBottom: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>{ucs.slice(0, 3).map((u, i) => <Bg key={i} color={[C.ac, C.gn, C.pu][i]}>{u.cat}</Bg>)}
        <button onClick={exportPptx} disabled={exporting} style={{ marginLeft: "auto", padding: "6px 16px", borderRadius: 7, border: "1px solid "+C.ac, background: exporting ? C.ag : C.ac, color: "#fff", fontSize: 12, fontWeight: 600, cursor: exporting ? "wait" : "pointer", fontFamily: "'DM Sans',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>{exporting ? "Generating..." : "\uD83D\uDCE5 Export PPTX"}</button>
      </div>
    </div>

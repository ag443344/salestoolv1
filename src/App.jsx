...
export default function App() {
  ...
  async function go() {
    ...
    try {
      ...
      setPh("Drafting email...");
      const email = await ai("Write a cold sales email for Allium, a blockchain data platform. Write ONLY the email text, nothing else. No subject line label, just the email body. 3-4 short paragraphs.", "Write a cold email to " + co.name + " (" + d + "). They are in " + co.industry + ". Their blockchain relevance: " + co.blockchainRelevance + ". Pain points: " + (co.painPoints||[]).join(", ") + ". Pitch: " + (pitch.headline||"") + ". Similar customers: " + custStr + ". Products to recommend: " + prods.map(p => p.name).join(", "));
      pitch.openingEmail = email;

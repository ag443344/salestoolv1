...
export default function App() {
  ...
  async function ai(sys, usr) {
    const r = await fetch("/api/claude", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, system: sys, messages: [{ role: "user", content: usr }], tools: [{ type: "web_search_20250305", name: "web_search" }] }) });
    const d = await r.json();
    const raw = d.content?.filter(b => b.type === "text").map(b => b.text).join("\n") || "";
    return raw.replace(/<\/?(?:cite|antml:cite)[^>]*>/g, "");
  }
  ...
  async function go() {
    ...
    try {
      ...
      const cr = await ai("Research prospects for Allium blockchain data platform. Return ONLY JSON, no markdown fences: {\"name\":\"...\",\"description\":\"...\",\"industry\":\"...\",\"blockchainRelevance\":\"...\",\"products\":[],\"painPoints\":[],\"size\":\"...\",\"keyInsight\":\"...\"}", "Research: " + d + ". Focus on blockchain/crypto data needs.");
      ...
      const sr = await ai("Find decision-makers for blockchain data sale. Return ONLY JSON array, no markdown: [{\"role\":\"...\",\"name\":\"...\",\"relevance\":\"...\",\"talkingPoints\":[],\"linkedinSearchQuery\":\"...\"}]", "Stakeholders at " + d + " (" + co.name + "), " + co.industry + ". Blockchain: " + co.blockchainRelevance);
      ...
      const pr = await ai("Write Allium pitch. Return ONLY JSON, no markdown: {\"headline\":\"...\",\"valueProps\":[{\"title\":\"...\",\"detail\":\"...\"}],\"competitiveEdge\":\"...\",\"openingEmail\":\"...\"}", "Company: " + co.name + ", " + d + ", " + co.description + ", " + co.industry + ", Blockchain: " + co.blockchainRelevance + ", Pain: " + JSON.stringify(co.painPoints) + ", Use Cases: " + ucs.map(u => u.cat).join(", ") + ", Proof: " + cust.map(c => c.n + " (" + c.u + ")").join("; ") + ", Products: " + prods.map(p => p.name).join(", "));
      let pitch; try { pitch = JSON.parse(pr.replace(/```json\n?|```/g, "").trim()); } catch { pitch = { headline: co.name + " needs blockchain data", valueProps: [{ title: "Unified data", detail: "One platform for 130+ chains." }], competitiveEdge: "Trusted by Visa, Stripe, Coinbase.", openingEmail: pr.slice(0, 500) }; }
  ...
  async function runViz(cat) {
    ...
    try {
      const r = await fetch("/api/claude", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 4096, system: "You are a data assistant. Use the Allium explorer_run_sql tool to execute SQL. After getting results, output ONLY the JSON data array.", messages: [{ role: "user", content: "Execute this SQL with explorer_run_sql:\n\n" + q.sql }], mcp_servers: [{ type: "url", url: "https://mcp-oauth.allium.so", name: "allium-mcp" }] }) });
      const j = await r.json();
      const data = extractData(j);
      if (data?.length) { setVd(data.map(row => { const nr = {}; for (const [k, v] of Object.entries(row)) nr[k.toUpperCase()] = v; return nr; })); }
